...
import { 
  MapPin, Clock, ShieldCheck, ArrowRight, Loader2, Cpu, Hexagon 
} from "lucide-react";
...
const BookingPage = () => {
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // --- ২. Email Invoice Challenge লজিক ---
        ...
        try {
          // এই আইডিগুলো ডামি রাখা হয়েছে যেন আপনার কোড এরর না দেয়।
          await emailjs.send(
            'service_dummy_id',   
            'template_dummy_id',  
            templateParams, 
  ...
  if (authLoading || loading) return (
    <div className="min-h-screen bg-[#02040a] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-amber-500 font-mono text-[10px] tracking-widest animate-pulse uppercase">Verifying_Credentials...</p>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7">
            <form onSubmit={handleBooking} className="bg-white/[0.03] backdrop-blur-3xl p-10 border border-white/10 rounded-3xl space-y-6 shadow-2xl relative overflow-hidden">
              <div className="space-y-4">
                ...
                <div className="flex justify-between font-mono text-xl font-black">
                  ...
                  <span className="text-amber-500">${duration * (service?.price || 0)}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-6">
                <div className="space-y-2">
                  <label className="text-[9px] font-mono text-gray-500 uppercase tracking-widest">Division</label>
                  <select className="w-full bg-black/40 border border-white/10 p-4 rounded-xl text-xs font-mono outline-none focus:border-amber-500 transition-all appearance-none" onChange={handleDivisionChange} required>
                    <option value="">SELECT DIVISION</option>
                    {Object.keys(locationData).map(div => <option key={div} value={div}>{div}</option>)}
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-mono text-gray-500 uppercase tracking-widest">District</label>
                  <select className="w-full bg-black/40 border border-white/10 p-4 rounded-xl text-xs font-mono outline-none focus:border-amber-500 disabled:opacity-30 transition-all" onChange={(e)=>setFormData({...formData, district: e.target.value})} required disabled={!formData.division}>
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-mono text-gray-500 uppercase tracking-widest">Detailed Address</label>
                <textarea placeholder="HOUSE NO / ROAD NO / BLOCK..." className="w-full bg-black/40 border border-white/10 p-4 rounded-xl text-xs h-24 font-mono outline-none focus:border-amber-500 resize-none transition-all" onChange={(e)=>setFormData({...formData, address: e.target.value})} required></textarea>
              </div>

              <button type="submit" disabled={submitting} className="w-full bg-amber-600 hover:bg-amber-500 py-5 rounded-2xl font-black uppercase text-xs transition-all flex items-center justify-center gap-3 text-black">
                {submitting ? <Loader2 className="animate-spin"/> : <>Initialize_Protocol <ArrowRight size={18}/></>}
              </button>
            </form>
          </div>
          ...
          <div className="lg:col-span-5 bg-white/[0.02] backdrop-blur-3xl p-10 border border-white/10 rounded-3xl h-fit sticky top-32 shadow-2xl">
             ...
             <p className="text-2xl font-black text-white uppercase italic mb-8 tracking-tighter">{service?.title}</p>
             <div className="space-y-4 border-t border-white/5 pt-8">
                <div className="flex justify-between text-xs font-mono text-gray-500">
                  <span>CLIENT:</span>
                  <span className="text-amber-500/80 font-bold">{user?.displayName || "UNIDENTIFIED_USER"}</span>
                </div>
                <div className="flex justify-between text-xs font-mono text-gray-500">
                  <span>STATUS:</span>
                  <span className="text-amber-200 font-bold uppercase animate-pulse">Waiting_Input</span>
                </div>
             </div>
             <div className="flex justify-between text-4xl font-black border-t border-amber-500/20 pt-10 mt-10 italic font-mono">
                 <span className="text-amber-500 text-[10px] self-center tracking-widest font-bold">TOTAL_ESTIMATE</span>
                 <span className="text-amber-500">${duration * (service?.price || 0)}</span>
             </div>
          </div>
        </div>
